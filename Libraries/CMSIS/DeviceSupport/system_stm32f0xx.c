...
void SystemInit (void)
{
  ...
  RCC->CFGR3 &= (unsigned int)0xFFFFFEAD;
...
{
  unsigned int tmp = 0, pllmull = 0, pllsource = 0, prediv1factor = 0;
...
/******************************************************************************/
  ...
  if (HSEStatus == (unsigned int)0x01)
  {
    ...
    /* PCLK = HCLK */
    RCC->CFGR |= (unsigned int)RCC_CFGR_PPRE_DIV1;

    /* PLL configuration */
    RCC->CFGR &= (unsigned int)((unsigned int)~(RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE | RCC_CFGR_PLLMULL));
    RCC->CFGR |= (unsigned int)(RCC_CFGR_PLLSRC_PREDIV1 | RCC_CFGR_PLLXTPRE_PREDIV1 | RCC_CFGR_PLLMULL6);
    ...
    while ((RCC->CFGR & (unsigned int)RCC_CFGR_SWS) != (unsigned int)RCC_CFGR_SWS_PLL)
    {
    }
  }
  else
